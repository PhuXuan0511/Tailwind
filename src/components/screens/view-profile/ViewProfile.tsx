    ...
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      if (currentUser) {
        try {
          ...
          if (userDoc.exists()) {
            const userData = userDoc.data() as Omit<UserProfile, 'displayName' | 'email' | 'photoURL' | 'uid'>;
      ...
      <div className="bg-white rounded-lg shadow-lg overflow-hidden max-w-4xl mx-auto">
        <div className="md:flex">
          <div className="md:w-1/3 bg-primary p-8 text-center">
            ...
            <p className="text-primary-content opacity-80">{user.role || 'User'}</p>
          </div>
          
          <div className="md:w-2/3 p-8">
            <h1 className="text-2xl font-bold mb-6 text-gray-800 border-b pb-2">User Profile</h1>
